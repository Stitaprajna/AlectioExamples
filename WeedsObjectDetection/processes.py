...
def getdatasetstate(args={}):
    return {k: k for k in range(3664)}
...
def train(args, labeled, resume_from, ckpt_file):
    ...
    ann_files = [os.path.join(yolo_args.data_dir, "annotations/{}_annotations.coco.json".format(x)) for x in splits]
    ...
    if resume_from is not None:
        ...
        if os.path.isfile(os.path.join(args["EXPT_DIR"], 'pretrained_model')):
            pre_trained_ckpt_path = os.path.join(args["EXPT_DIR"], 'pretrained_model')
            checkpoint = torch.load(pre_trained_ckpt_path, map_location=device)['model'] # load last checkpoint
...
def test(args, ckpt_file):
    ...
    file_root = os.path.join(yolo_args.data_dir, "images", "test")
    ann_file = os.path.join(yolo_args.data_dir, "annotations", "test_annotations.coco.json")
    if not os.path.isdir(args["EXPT_DIR"]):
        os.makedirs(args["EXPT_DIR"], exist_ok=True)
...
def infer(args, unlabeled, ckpt_file=None):
    ...
    dataset = yolo_args.dataset
    file_root = os.path.join(yolo_args.data_dir, "images", "train")
    ann_file = os.path.join(yolo_args.data_dir, "annotations", "train_annotations.coco.json")
    if not os.path.isdir(args["EXPT_DIR"]):
        os.makedirs(args["EXPT_DIR"], exist_ok=True)
...
if __name__ == '__main__':
    ...
    args['cur_loop']=0
    train(args=args, labeled=list(range(3664)), resume_from=None, ckpt_file='ckpt')
