...
def update_datamap(original_datamap, new_datamap):
    if original_datamap is None:
        return new_datamap
    # Validate all records in original_datamap with new_datamap /home/stitaprajna/Agritech_Demo/data/coco/annotations/instances_train2017.json
    validated_indices = []
...
def train(args, labeled, resume_from, ckpt_file):
    ...
    if DALI:
        ...
        # DALICOCODataLoader behaves like PyTorch's DataLoader. /home/stitaprajna/Agritech_Demo/data/coco/annotations/instances_train2017.json.json
...
def test(args, ckpt_file):
    ...
    ann_file = os.path.join(yolo_args.data_dir, "annotations", "instances_test.json")
    if not os.path.isdir(args["EXPT_DIR"]):
        os.makedirs(args["EXPT_DIR"], exist_ok=True)
...
def infer(args, unlabeled, ckpt_file=None):
    ...
    ann_file = os.path.join(yolo_args.data_dir, "annotations", "instances_train.json")
    if not os.path.isdir(args["EXPT_DIR"]):
        os.makedirs(args["EXPT_DIR"], exist_ok=True)
